...
(async () => {
  ...
  */
    ...
    // create metadata for the collection and return the uri
    async function createCollectionNftURI() {
      ...
      const CONFIG = {
        ...
        imgFileName: "receiptImage.png",
        imgType: "image/png",
        imgName: "swissDAO Receipt Token",
        description: "swissDAO Receipt Token!",
        attributes: [
  ...
  */
    ...
    const uploadNFTMetadataURI = async () => {
      ...
      // UPLOAD YOUR OWN METADATA URI
      const NFT_CONFIG = {
        name: "swissDAO Receipt Token",
        description: "Itemized Receipt from swissDAO Solana Ecosystem Day!",
        attributes: [
    ...
    console.log(
      `Minting a single receipt compressed NFT to ${payer.publicKey.toBase58()}...`,
    );
    ...
    console.log(
      `Minting a single receipt compressed NFT to ${testWallet.publicKey.toBase58()}...`,
    );
    ...
    console.log(`===============================`);
    console.log(
      "Total cost for Receipts:",
      numberFormatter((initBalance - balance) / LAMPORTS_PER_SOL, true),
      "SOL\n",
    );
    console.log(`===============================`);
    console.log('SAVE THESE VALUES IN YOUR .env FILE');
    console.log(`===============================`);
    console.log(`NEXT_PUBLIC_RECEIPT_METADATA_URI=${nft_metadata_uri}`);
    console.log(`NEXT_PUBLIC_RECEIPT_TREE_ADDRESS=${tree.treeAddress.toBase58()}`);
    console.log(`NEXT_PUBLIC_RECEIPT_TREE_AUTHORITY=${tree.treeAuthority.toBase58()}`);
    console.log(`NEXT_PUBLIC_RECEIPT_COLLECTION_MINT=${collection.mint.toBase58()}`);
    console.log(`NEXT_PUBLIC_RECEIPT_COLLECTION_METADATA_ACCOUNT=${collection.metadataAccount.toBase58()}`);
    console.log(`NEXT_PUBLIC_RECEIPT_COLLECTION_MASTER_EDITION_ACCOUNT=${collection.masterEditionAccount.toBase58()}`);
    console.log(`===============================`);
  } catch (error) {
    console.error(error);
  }
