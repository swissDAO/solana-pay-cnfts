...
export default function Home() {
  ...
  const handleGenerateQR = async () => {
    ...
    const order_products = order?.products
      ...
      .join("+");
    const order_products_quantity = order?.products
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        const signatureInfo = await findReference(solanaConnection, reference, {
        ...
        const parsed_transaction = await solanaConnection.getParsedTransactions(
          [mostRecentNotifiedTransaction.current!],
          "confirmed",
        );

        // parse parsed_transaction[0]?.transaction?.message.accountKeys for where signer: true and get the public key
        const signer_of_transaction =
          parsed_transaction[0]?.transaction?.message.accountKeys
            .filter((account: any) => account.signer)[0]
            .pubkey.toBase58();

        // get the transfered amount from the parsed transaction
        
        const transfered_amount =
          parsed_transaction[0]?.transaction?.message.instructions.filter(
            (program: any) => program.program === "spl-token",
            // @ts-ignore
          )[0].parsed.info.tokenAmount.uiAmount;
      ...
      notify(`Transaction verified, you spent ${paymentConfirmation?.amount} USDC.
        ${
          parseFloat(paymentConfirmation?.amount) >= parseFloat("10.00")
            ...
            : `Spend ${
                parseFloat("10.00") - parseFloat(paymentConfirmation?.amount)
              } more USDC to receive a coupon next time!`
        }`);
        ...
        <div>
          ...
          <div className="flex-col z-10 w-full max-w-5xl items-center justify-between font-mono text-sm lg:flex">
            ...
            <button
              ...
              onClick={() => {
                !qrActive ? handleGenerateQR() : handleClearQR();
              }}
            >
