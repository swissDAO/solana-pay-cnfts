...
const check_for_coupons = async (buyerPublicKey: PublicKey) => {
  ...
  .then(res => {
    ...
    console.log("Assets returned:", res.items.map(asset => asset.compression.tree));
    ...
    res.items
      ...
      .map(asset => {
        ...
        if (asset.compression.tree === treeAddress.toBase58() && asset.ownership.owner === BUYER_WALLET_ADDRESS){
...
async function post(
  ...
  try {
    ...
    const message = discount === 0.5 ? "50% Discount!" : "Thanks for your order! 🤑"
