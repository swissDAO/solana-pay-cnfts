...
const dev_connection = new Connection(
...
const verify_asset_ownership = async (
  asset: PublicKey,
  buyerPublicKey: PublicKey,
) => {
      ...
      res.items
        ...
        .map(asset => {
          ...
          if (
            ...
            asset.ownership.owner === buyerPublicKey.toBase58()
          )
...
async function post(
  ...
  try {
    ...
    if (verified_asset_ids.length! >= 1) {
      try {
        ...
        const transferInstruction = await createCnftTransferInstruction(
          verified_asset_ids[0],
          buyerPublicKey,
        );
        printConsoleSeparator(
          `Sending the transfer transaction for asset_id: ${verified_asset_ids[0].toBase58()}...`,
        );
    ...
    const message =
      verified_discount === 0.5 ? '50% Discount!' : 'Thanks for your order! 🤑';
